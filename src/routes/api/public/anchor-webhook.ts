...
interface AnchorAttributes {
  ...
  virtualNuban?: { accountNumber?: string };
  destinationAccount?: { accountNumber?: string };
}

interface AnchorWebhookPayload extends AnchorAttributes {
  event?: string;
  data?: AnchorAttributes & {
    attributes?: AnchorAttributes;
    relationships?: {
      virtualNuban?: { data?: { id?: string } };
    };
  };
}
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        let payload: AnchorWebhookPayload;
        try {
        ...
        const attrs: AnchorAttributes = payload?.data?.attributes ?? payload?.data ?? payload;
